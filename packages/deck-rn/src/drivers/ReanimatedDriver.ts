...
import { SharedValue, withTiming, withDelay, Easing, cancelAnimation, runOnJS } from 'react-native-reanimated';

type EasingFn = (value: number) => number;

type CardAnimationHandle = {
...
type CardAnimationState = CardAnimationHandle & {
  isFaceUp: boolean;
};
...
export class ReanimatedDriver implements AnimationDriver {
  ...
  register(cardId: CardId, handle: CardAnimationHandle, initialFaceUp = false): void {
